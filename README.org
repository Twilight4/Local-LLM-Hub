#+title: Local LLM Hub
#+subtitle: Local, containerized AI infrastructure — Ollama + LiteLLM + Qdrant (RAG over MCP)
#+author: Twilight4
#+date: [2026-07-16 Wed]
...
* Project Overview
Three services plus a small retrieval layer:
...
├── docker-compose.yml      ollama + litellm + qdrant, GPU hand-off, healthchecks
...
- *Disk:* ~5 GB (images + chat model + embedding model).
...
3. *Start the stack* (pulls the images on first run; starts ollama + litellm + qdrant):
   #+begin_src bash
   docker compose up -d
   #+end_src
...
4. *Wait for all services to be healthy:*
   #+begin_src bash
   docker compose ps       # ollama, litellm, qdrant all show (healthy)
   #+end_src
...
docker compose down -v     # also delete ollama-data + qdrant-data (re-pull / re-index next time)
#+end_src
