...
- [[#deploy-infrastructure][Deploy Infrastructure]]
  ...
  - [[#deploy-on-kubernetes-kind][Deploy on Kubernetes (kind)]]
...
Re-render after topology changes: =uv run assets/render_architecture.py= (matplotlib; the script is committed alongside the PNG).
...
├── deploy/
│   ├── kind-config.yaml      kind cluster def + localhost→NodePort mappings
│   ├── Makefile              helm lifecycle (create/install/upgrade/verify/logs/destroy) + gitops-* targets
│   ├── gitops/application.yaml  ArgoCD Application CR (auto-syncs the chart from main)
│   └── helm/local-llm-hub/   the Helm chart (Chart.yaml, values.yaml, templates/*)
└── assets/
    ├── architecture.png        logical request & tracing flow (see Architecture)
    ├── render_architecture.py  renders architecture.png  (uv run)
    ├── k8s_topology.png        K8s deployment topology (see Deploy on Kubernetes)
    ├── render_k8s_topology.py  renders k8s_topology.png  (uv run)
    ├── gitops_topology.png     GitOps control flow (see GitOps (ArgoCD))
    └── render_gitops_topology.py  renders gitops_topology.png  (uv run)
#+end_example
...
- *Python tooling:* [[https://github.com/astral-sh/uv][uv]] — a fast Python runner (a single Rust binary, not a pip package); the =rag/= scripts declare deps inline and run via =uv run=.
  ...
  #+end_src
- Tools for kubernetes deployment: =kind=, =helm=, =kubectl=
...
The same five-service stack runs on a local [[https://kind.sigs.k8s.io/][kind]] (Kubernetes-in-Docker) cluster via a Helm chart under =deploy/helm/local-llm-hub/=.

*Note:* Ollama runs *CPU-only* in this cluster — AMD GPU passthrough under kind is out of scope here. (The Docker Compose stack keeps its optional GPU override.)

#+caption: Deployment topology on kind — Pods, NodePort Services, PVCs, initContainer ordering, and the post-install model-pull Job. Ollama is internal-only (no localhost mapping).
[[file:assets/k8s_topology.png]]

*** Quick start
Run everything from the repo's =deploy/= directory:

#+begin_src shell
cd deploy
cp helm/local-llm-hub/values.local.yaml.example helm/local-llm-hub/values.local.yaml
KEY=sk-$(openssl rand -hex 32); sed -i "s/sk-REPLACE_WITH_64_HEX_CHARS/$KEY/" helm/local-llm-hub/values.local.yaml
make create install
make verify
#+end_src

=make install= blocks until every pod is =Ready= (Helm =--wait=), then runs a post-install =Job= that pulls the Ollama models (=llama3.2= ~2 GB + =nomic-embed-text=) into the =ollama-models= volume. First run takes several minutes while the models download.

*** What gets deployed
- A single-node kind cluster named =local-llm-hub=, namespace =local-llm-hub=.
- Five single-replica (=replicas: 1=) Deployments: =ollama=, =litellm=, =qdrant=, =phoenix=, =open-webui=.
- Four PVCs: =ollama-models=, =qdrant-data=, =phoenix-data=, =open-webui-data=.
- A =masterKey= Secret (=local-llm-hub-secrets=) and a LiteLLM =ConfigMap= (=litellm-config=).
- Every container image is pinned by digest (=repo:tag@sha256:…=), so deploys are reproducible regardless of tag drift.

*** Localhost access
Four services are mapped from the kind node to the host:

| Service    | Port | URL                   |
|------------+------+-----------------------|
| Open WebUI | 8080 | http://127.0.0.1:8080 |
| LiteLLM    | 4000 | http://127.0.0.1:4000 |
| Phoenix    | 6006 | http://127.0.0.1:6006 |
| Qdrant     | 6333 | http://127.0.0.1:6333 |

*Ollama (11434) is internal-only.* The host already runs a native =ollama.service= on =127.0.0.1:11434=, so the cluster's Ollama is /not/ exposed on localhost — it is reachable only inside the cluster (ClusterIP =ollama:11434=). For ad-hoc debugging from the host:

#+begin_src shell
kubectl -n local-llm-hub port-forward svc/ollama 11434:11434
#+end_src

*** Lifecycle targets
All targets run from the =deploy/= directory (=cd deploy && make …=):

| Target       | Action                                                      |
|--------------+-------------------------------------------------------------|
| =make create=  | Create the kind cluster (=local-llm-hub=)                     |
| =make install= | =helm install= with =--wait= (pods Ready) + post-install models |
| =make upgrade= | =helm upgrade= with =--wait=                                    |
| =make verify=  | Wait for pods, then liveliness + one-token chat check       |
| =make logs=    | Tail the last 80 lines from every service pod               |
| =make destroy= | Uninstall the release and delete the kind cluster           |

** GitOps (ArgoCD)
#+caption: GitOps control flow — git main → ArgoCD (ns argocd) auto-syncs the chart (prune + selfHeal) into ns local-llm-hub; the master-key Secret is applied out-of-band (never in git, invisible to ArgoCD); manual drift is self-healed.
[[file:assets/gitops_topology.png]]

Instead of =make install= (direct Helm), ArgoCD reconciles the chart from =main=:

#+begin_src shell
cd deploy
make destroy && make create
make gitops-bootstrap        # install ArgoCD (pinned v3.4.5) + apply the Application
make gitops-secret           # inject the master key out-of-band (kept out of git)
make gitops-verify           # Synced + Healthy; chat works
#+end_src

ArgoCD syncs the chart anonymously (the repo is public) with =prune= +
=selfHeal=, so any manual drift (=kubectl scale=, edits) is reverted to the
declared state in =main=. The master-key =Secret= is applied out-of-band and is
*not* managed by ArgoCD, so it never lands in git. The ArgoCD dashboard:
...
* Using the API
The gateway is OpenAI-compatible. This section covers programmatic / scripted access, which the =rag/= scripts depend on. Authenticate with the master key from =.env=.
...
- [X] Kubernetes: package the five-service stack as a Helm chart under =deploy/helm/=.
- [X] GitOps: ArgoCD =Application= auto-syncs the chart from =main= to the cluster.
