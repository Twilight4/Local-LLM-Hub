...
| Orchestration   | Docker Compose              | Amazon ECS (Fargate) or EKS                   |
...
- [ ] RAG: token-aware chunking + Qdrant metadata filters (=source_path=, =ingested_at=) for better retrieval.
- [ ] Web UI: Open WebUI container pointed at LiteLLM for chat from a browser.
- [ ] Streaming responses: =stream: true= support for chat completions.
